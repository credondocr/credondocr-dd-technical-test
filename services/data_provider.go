...
// This is the struct data from provider
type Data struct {
...
func (c *CustomTime) UnmarshalJSON(v []byte) error {
	var err error
	c.Time, err = time.Parse(utils.DefaultLayout, strings.ReplaceAll(string(v), "\"", ""))
	if err != nil {
		return err
	}
	return nil
...
func getSingleDayRequest(date string) ([]Data, error) {
...
func getMonthRequest(from time.Time) ([]Data, error) {
	config := config.GetConfig()
